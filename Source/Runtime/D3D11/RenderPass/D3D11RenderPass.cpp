...
namespace Hollow
{
	D3D11RenderPass::D3D11RenderPass(const RenderPassDesc& desc, ID3D11Device* pDevice) : RenderPass(desc)
	{
		// I will use one rtv for now. I may change this later
		// Reserve space for the rtv
		mD3DRenderTargetViews.reserve(desc.ColorAttachments.size());

		D3D11_RENDER_TARGET_VIEW_DESC rtvDesc = {};
		rtvDesc.Format = D3D11TextureUtils::GetDXTextureFormat(desc.ColorAttachments[0].pTexture->GetFormat());

		if (GetOwnerDevice()->GetSwapchain()->GetSampleCount() > 1)
		{
		...
		ID3D11Resource* pTexture = std::dynamic_pointer_cast<D3D11Texture>(desc.ColorAttachments[0].pTexture)->GetD3DTexture().Get();

		pDevice->CreateRenderTargetView(pTexture, &rtvDesc, &mD3DRenderTargetViews[0]);
		...
		ID3D11Resource* pDepthStencilTexture = std::dynamic_pointer_cast<D3D11Texture>(desc.DepthStencilAttachment.pTexture)->GetD3DTexture().Get();

		pDevice->CreateDepthStencilView(pDepthStencilTexture, &dsvDesc, &mD3DDepthStencilView);
		...
		else
		{
			...
			srvDesc.Texture2D.MipLevels = desc.ColorAttachments[0].MipLevel;
