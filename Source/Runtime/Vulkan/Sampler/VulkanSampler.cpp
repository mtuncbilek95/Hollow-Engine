...
namespace Hollow
{
	VulkanSampler::VulkanSampler(const SamplerDesc& desc, VulkanDevice* pDevice) : Sampler(desc, pDevice), 
		mVkDevice(pDevice->GetVkDevice()), mVkPhysicalDevice(pDevice->GetVkPhysicalDevice()), mVkSampler(VK_NULL_HANDLE)
	{
		...
		samplerInfo.compareOp = VulkanCommonUtils::GetVkCompareOperation(desc.CompareOp);
		samplerInfo.mipmapMode = VulkanSamplerUtils::GetVkMipmapMode(desc.MapMode);
		samplerInfo.mipLodBias = desc.MipLODBias;
		samplerInfo.minLod = desc.MinLOD;
		samplerInfo.maxLod = desc.MaxLOD;

		CORE_ASSERT(vkCreateSampler(mVkDevice, &samplerInfo, nullptr, &mVkSampler) == VK_SUCCESS, "VulkanSampler", "Failed to create VulkanSampler");
